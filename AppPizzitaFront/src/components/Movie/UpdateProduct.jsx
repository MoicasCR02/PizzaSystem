...
export function UpdateProduct() {
  ...
  const id_producto = routeParams.id || null;
  //Valores a precargar en el formulario, vienen del API
  const [values, setValores] = useState(null);
  const [previewImage, setPreviewImage] = useState(null); // URL de la imagen precargada
  // URL base del servidor de imágenes
  const BASE_IMAGE_URL = import.meta.env.VITE_BASE_URL + "uploads";
  //Obtener el producto del API
  useEffect(() => {
    //Llamar al API y obtener un producto
    if (id_producto != undefined && !isNaN(Number(id_producto))) {
      ProductService.getProductById(id_producto)
        .then((response) => {
          console.log(response.data);
          setError(response.error);
          //Dar formato a ingredientes
          const ingredientes = response.data.ingredientes.map(
            (item) => item.id_ingrediente
          );
        ...
        ProductService.updateProduct(formData)
          .then((response) => {
            if (response.data != null) {
              toast.success(
                `Producto modificado #${response.data.id_producto} - ${response.data.nombre_producto}`,
                {
                  duration: 4000,
                  position: "top-center",
                }
